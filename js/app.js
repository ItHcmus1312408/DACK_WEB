(function() {
	...
	app.controller('CurriculumCtrl', ['$scope', '$firebase', '$filter',
		function($scope, $firebase, $filter) {

			var cv = this;	
			cv.information = {};
			var ref = new Firebase("https://curriculumvitae.firebaseio.com");  
			...
			cv.register = function(){
				...
				if (error) {
					switch (error.code) {
						case "EMAIL_TAKEN":
						cv.message = "The new user account cannot be created because the email is already in use.";
						break;
						case "INVALID_EMAIL":
						cv.message = "The specified email is not a valid email.";
						break;
						default:
						//cv.message = "Error creating user:" + error;
					}
				} else {
					cv.message = "Successfully created user account";}})
			}
